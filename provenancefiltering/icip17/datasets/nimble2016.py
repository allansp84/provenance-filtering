...
class Nimble2016(Dataset):
    ...
    def _build_meta(self, in_path, file_types, random_state=None, fold=None, check_files=False):
        ...
        # fin = open(NC2016_MANIPULATION, 'r')
        # csv_manipulation = np.array([line.split('\n')[0].split('|') for line in fin.readlines()])
        # fin.close()
        #
        # fin = open(NC2016_REMOVAL, 'r')
        # csv_removal = np.array([line.split('\n')[0].split('|') for line in fin.readlines()])
        # fin.close()
        #
        # fin = open(NC2016_SPLICE, 'r')
        # csv_splice = np.array([line.split('\n')[0].split('|') for line in fin.readlines()])
        # fin.close()

        # fin = open(NC2016_FILTERED_SPLICE, 'r')
        # csv_splice = np.array([line.split('\n')[0].split('|') for line in fin.readlines()])
        # fin.close()
        #
        # filtered_csv = np.array([[line[1],
        #                           os.path.splitext(os.path.basename(line[5]))[0],
        #                           os.path.splitext(os.path.basename(line[15]))[0]] for line in csv_splice[1:]])

        filtered_csv = []
        fnames = sorted(glob(os.path.join(NIMBLE_ANNOTATION_PATH, '*.txt')))
        for fname in fnames:
            data = open(fname, 'r').read().splitlines()

            ProbeFileID = os.path.splitext(data[0])[0]

            DonorFileID_1 = os.path.splitext(data[4].split(', ')[0])[0]
            DonorFileID_2 = os.path.splitext(data[4].split(', ')[1])[0]

            BaseFileName_1 = os.path.splitext(data[2].split(', ')[0])[0]
            BaseFileName_2 = os.path.splitext(data[2].split(', ')[1])[0]

            ProbeMask = os.path.splitext(data[1].split(', ')[0])[0]

            filtered_csv += [[ProbeFileID, DonorFileID_1, BaseFileName_1, ProbeMask]]
            filtered_csv += [[ProbeFileID, DonorFileID_2, BaseFileName_2, ProbeMask]]
        ...
        # np.savetxt('gt-allan.txt', filtered_csv, fmt='%s', delimiter='|', header='probe|alien|host')
        hash_idx = {}
        hash_idx_probe = {}
        hash_idx_world = {}
        ...
        for i, folder in enumerate(folders):
            ...
            for fname in fnames:
                ...
                pos_class = int(True if 'probe' in rel_path else False)
                ...
                if 'probe' in rel_path:
                    ...
                    hash_idx.update({img_id: img_idx})
